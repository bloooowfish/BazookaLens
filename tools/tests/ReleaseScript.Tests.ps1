...
Assert-Match -Actual $configText -Pattern "RepoName = 'BazookaLens'" -Message 'Release config should target BazookaLens.'
Assert-Match -Actual $configText -Pattern "ProjectPath = 'BazookaLens\\BazookaLens\.csproj'" -Message 'Release config should point at the plugin project.'
Assert-Match -Actual $configText -Pattern "TestProjectPath = 'BazookaLens\.Tests\\BazookaLens\.Tests\.csproj'" -Message 'Release config should point at plugin tests.'
Assert-Match -Actual $configText -Pattern "ReleaseAssetName = 'BazookaLens-\{0\}\.zip'" -Message 'Release config should keep the expected asset name.'
...
Assert-Match -Actual $configText -Pattern 'github-bf:bloooowfish/BazookaLens\.git' -Message 'Release config should require the subaccount SSH alias remote.'
